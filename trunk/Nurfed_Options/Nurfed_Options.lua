...
for _, name in ipairs(sort) do
  local info = panels[name]
  if not info.addon or IsAddOnLoaded(info.addon) then
    local panel = Nurfed:create("Nurfed_"..name, "uipanel")
    panel.name = name
    panel.parent = "Nurfed"
    panel.default = function(self)
        if InterfaceOptionsFrameAddOns.selection == self then
          ChatFrame1:AddMessage("Default: "..InterfaceOptionsFrameAddOns.selection.name)
        end
      end
    getglobal("Nurfed_"..name.."Title"):SetText(info.title)
    getglobal("Nurfed_"..name.."SubText"):SetText(info.subtext)

    for opt, tbl in pairs(info.menu) do
      Nurfed:create("Nurfed_"..name..opt, tbl, panel)
    end
